...
def trendDataframe(urlReceived, breakPointNumber):
    ...
    url = urlReceived
    soupt = makeSoupTrend(url, 0)
    page = soupt.findAll('div', {'class': re.compile("productlist__product js-ovgrid-item")})
    pl = len(page)
    while order < breakPointNumber:
        if order > 0:
            temp = soupt.find('a', {'class': re.compile('pagination__next jsClickTrack')}).get('href').rsplit('=', 1)
            new = temp[0] + '=' + str(pl)
            pl = pl + len(page)
            soup = makeSoupTrend(new, 1)
            page = soup.findAll('div', {'class': re.compile("productlist__product js-ovgrid-item")})
        for product in page:
            order = order + 1
            urlt = 'https://www.soliver.eu' + product.findAll('a', {'class': re.compile("plproduct jsPLProduct js-ovlistview-productdetaillink")})[0].get('href')
...
def performScraping(urlReceived, folderName, breakPointNumber):
    ...
    while breaki < breakPointNumber:
        ...
        try:
            np = page[i].find('a', {'class': re.compile("plproduct jsPLProduct js-ovlistview-productdetaillink")}).get('href')  # Find parent node of img
            if ('%.12s' % np) != 'https://www.':
