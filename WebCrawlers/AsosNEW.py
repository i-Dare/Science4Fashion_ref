...
def performScraping(urlReceived, folderName, breakPointNumber):
    ...
    engine = sqlalchemy.create_engine(constants.CONNECTION + 'SocialMedia')
    numUrlExist = 0
    numUrlSkipped = 0
    numImagesDown = 0
    ...
    while breaki < breakPointNumber:
        ...
        if not exists:
            ...
            folderIndividualName = cwd + "\\" + imgSiteFolder + '\\' + folderName + '\\'  # Creates the path where the images will be stored
            ...
            imagepath = (cwd + "\\" + imgSiteFolder + '\\' + folderName + '\\' + nameOfFile + ImageType)
            empPhoto = convertToBinaryData(imagepath)
...
if __name__ == '__main__':
    ...
    for i in range(0, len(lines)):
        ...
        folderName = getFolderName(keys).replace(" ", "")
